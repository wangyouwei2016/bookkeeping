...
const LoginScreen = ({ onLogin, onResetConfig }: { onLogin: (user: UserType) => void, onResetConfig: () => void }) => {
  return (
    <div className="min-h-screen bg-gradient-to-br from-brand-50 to-brand-100 flex flex-col items-center justify-center p-8 text-center relative overflow-hidden">
      ...
      <h1 className="text-5xl font-bold text-gray-800 mb-4 z-10 tracking-tight">夫妻账本</h1>
      <p className="text-gray-500 mb-16 text-xl z-10 font-medium">请选择您的身份，开始共同记账</p>
      
      <div className="space-y-6 w-full max-w-sm z-10">
        <button 
          onClick={() => onLogin('husband')}
          className="w-full py-8 bg-white hover:bg-blue-50 border-2 border-white hover:border-blue-100 text-gray-800 rounded-3xl shadow-lg flex items-center justify-center gap-5 transition-all active:scale-[0.98]"
        >
        ...
        <button 
          ...
          className="w-full py-8 bg-white hover:bg-pink-50 border-2 border-white hover:border-pink-100 text-gray-800 rounded-3xl shadow-lg flex items-center justify-center gap-5 transition-all active:scale-[0.98]"
        >
...
const Dashboard = ({ 
  ...
  return (
    <div className="p-6 space-y-10">
      <header className="flex justify-between items-center pt-2">
        <div>
           ...
           <p className="text-base text-gray-500 mt-2 font-medium">
             当前身份: <span className="font-bold text-brand-600 bg-brand-50 px-2 py-0.5 rounded-md">{currentUser === 'husband' ? '丈夫' : '妻子'}</span>
           </p>
        </div>
      ...
      {/* Overview Card */}
      <div className="bg-gradient-to-br from-brand-900 to-brand-600 rounded-[2.5rem] p-10 text-white shadow-2xl shadow-brand-200/50 relative overflow-hidden group">
        ...
        <div className="relative z-10">
          <div className="text-brand-100 text-lg font-medium mb-3">家庭总结余</div>
          <div className="text-6xl font-extrabold mb-10 tracking-tight">
          ...
          <div className="flex gap-6">
            <div className="flex-1 bg-white/10 rounded-3xl p-5 backdrop-blur-sm border border-white/10">
              <div className="text-brand-100 text-sm font-medium mb-1">本月收入</div>
              <div className="font-bold text-3xl text-emerald-300">
                {isLoading ? '...' : `+${monthIncome.toFixed(0)}`}
              </div>
            </div>
            <div className="flex-1 bg-white/10 rounded-3xl p-5 backdrop-blur-sm border border-white/10">
              <div className="text-brand-100 text-sm font-medium mb-1">本月支出</div>
...
const AddTransaction = ({ onAdd, currentUser, isSaving }: { onAdd: (t: Transaction) => void, currentUser: UserType, isSaving: boolean }) => {
  ...
  return (
    <div className="p-6 pb-32">
      <h2 className="text-3xl font-bold text-gray-800 mb-8 pt-2">记一笔</h2>
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-brand-100 mb-10 relative overflow-hidden">
        ...
        <label className="text-sm font-bold text-brand-600 uppercase tracking-wider mb-3 block flex items-center gap-2">
        ...
        <div className="flex gap-4">
          <input
            ...
            className="flex-1 bg-gray-50 border-none rounded-2xl text-lg px-5 py-4 focus:ring-2 focus:ring-brand-200"
          />
          <button 
            type="button"
            onClick={toggleListening}
            className={`p-4 rounded-2xl transition-colors ${isListening ? 'bg-red-500 text-white animate-pulse' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
          >
            <Mic size={28} />
          </button>
          <button 
            ...
            className="p-4 bg-brand-600 text-white rounded-2xl hover:bg-brand-700 disabled:opacity-50"
          >
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="flex items-center gap-6 bg-gray-50 p-5 rounded-3xl border border-dashed border-gray-200">
           <span className="text-lg text-gray-500 font-bold">记账人:</span>
        ...
        <div className="flex gap-5">
            <button
              ...
              className={`flex-1 py-5 border-2 rounded-2xl font-bold text-xl transition-all ${
        ...
        <div>
          <label className="block text-base font-bold text-gray-500 mb-3 ml-1">金额</label>
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-20 pr-8 py-10 text-6xl font-extrabold text-gray-800 bg-gray-50 rounded-3xl border-none focus:ring-4 focus:ring-brand-500/20 outline-none placeholder-gray-200 tracking-tight"
              placeholder="0"
              required
            />
          </div>
        ...
        <div className="grid grid-cols-2 gap-6">
          <div>
            <label className="block text-base font-bold text-gray-500 mb-3 flex items-center gap-2 ml-1"><Tag size={20}/> 分类</label>
            <div className="relative">
               <select
                value={category}
                onChange={(e) => setCategory(e.target.value)}
                className="w-full p-5 bg-white border border-gray-200 rounded-2xl text-xl font-bold text-gray-800 focus:ring-2 focus:ring-brand-500 appearance-none shadow-sm"
              >
          ...
          <div>
            <label className="block text-base font-bold text-gray-500 mb-3 flex items-center gap-2 ml-1"><Calendar size={20}/> 日期</label>
            <input
              type="date"
              value={date}
              onChange={(e) => setDate(e.target.value)}
              className="w-full p-5 bg-white border border-gray-200 rounded-2xl text-xl font-bold text-gray-800 focus:ring-2 focus:ring-brand-500 shadow-sm"
            />
          </div>
        </div>

        <div>
          <label className="block text-base font-bold text-gray-500 mb-3 flex items-center gap-2 ml-1"><FileText size={20}/> 备注 (选填)</label>
          <input
            type="text"
            value={note}
            onChange={(e) => setNote(e.target.value)}
            placeholder="这笔钱是干嘛的？"
            className="w-full p-5 bg-white border border-gray-200 rounded-2xl text-xl text-gray-800 focus:ring-2 focus:ring-brand-500 shadow-sm placeholder-gray-400"
          />
        </div>
        ...
        <button
          type="submit"
          disabled={isSaving}
          className="w-full py-6 bg-gray-900 text-white rounded-3xl font-bold text-2xl shadow-xl shadow-gray-400/40 hover:bg-black active:scale-[0.98] transition-all disabled:opacity-70 flex items-center justify-center gap-4 mt-6"
        >
  ...
  return (
    <div className="p-6 space-y-8">
      <header className="flex justify-between items-center pt-2">
         ...
         <div className="flex bg-gray-100 p-2 rounded-2xl">
            <button 
              ...
              className={`px-5 py-2 text-base font-bold rounded-xl transition-all ${timeFilter === 'month' ? 'bg-white shadow text-brand-600' : 'text-gray-500'}`}
            >
              月度
            </button>
            <button 
              onClick={() => setTimeFilter('year')}
              className={`px-5 py-2 text-base font-bold rounded-xl transition-all ${timeFilter === 'year' ? 'bg-white shadow text-brand-600' : 'text-gray-500'}`}
            >
              年度
            </button>
         </div>
      ...
      <div className="flex items-center justify-between bg-white p-5 rounded-3xl border border-gray-100 shadow-sm">
         ...
         }} className="p-3 text-gray-400 hover:text-gray-600 active:scale-90 transition-transform bg-gray-50 rounded-xl">{'<'}</button>
      ...
      <div className="grid grid-cols-2 gap-5">
         <div className="bg-emerald-50 p-6 rounded-3xl border border-emerald-100">
            <div className="text-base text-emerald-600 mb-3 font-bold">总收入</div>
            <div className="text-3xl font-extrabold text-emerald-700">+{totalIncome.toFixed(0)}</div>
         </div>
         <div className="bg-rose-50 p-6 rounded-3xl border border-rose-100">
            <div className="text-base text-rose-600 mb-3 font-bold">总支出</div>
      ...
      {expenseData.length > 0 || totalIncome > 0 ? (
        <>
          <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100">
            <h3 className="text-base font-bold text-gray-500 uppercase tracking-wider mb-8">收支对比</h3>
            <div className="h-80 w-full">
               <ResponsiveContainer width="100%" height="100%">
                  <BarChart layout="vertical" data={userStatsData} barGap={8} margin={{ left: 10, right: 10, top: 10, bottom: 10 }}>
                    <XAxis type="number" hide />
                    <YAxis dataKey="name" type="category" axisLine={false} tickLine={false} width={50} style={{fontSize: '16px', fontWeight: 'bold'}} />
                    <RechartsTooltip formatter={(value: number) => `¥${value.toFixed(2)}`} cursor={{fill: 'transparent'}} contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)' }} />
          ...
          <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100">
            <h3 className="text-base font-bold text-gray-500 uppercase tracking-wider mb-8">支出分类占比</h3>
            <div className="h-80 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                    ...
                    <RechartsTooltip formatter={(value: number) => `¥${value.toFixed(2)}`} contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)' }}/>
                    <Legend verticalAlign="bottom" height={40} iconType="circle" wrapperStyle={{ fontSize: '14px', fontWeight: 'bold', paddingTop: '20px' }} />
                  </PieChart>
                </ResponsiveContainer>
            </div>
          </div>
