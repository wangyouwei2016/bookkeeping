    ...
    <div className="w-full h-[100dvh] bg-gray-50 lg:min-h-screen lg:h-auto lg:flex lg:justify-center lg:items-center font-sans overflow-hidden">
      ...
      */}
      <div className="w-full h-full flex flex-col relative bg-gray-50
                      lg:w-[400px] lg:h-[850px] lg:rounded-[3rem] lg:shadow-2xl lg:border lg:border-gray-200 lg:overflow-hidden">
        ...
        <main className="flex-1 w-full relative overflow-y-auto no-scrollbar scroll-smooth">
          ...
          <div className="pb-[calc(7rem+env(safe-area-inset-bottom))] min-h-full">
            {children}
          </div>
        </main>
        ...
        <nav className="absolute bottom-0 left-0 right-0 w-full bg-white/95 backdrop-blur-md border-t border-gray-100 z-50 pb-[env(safe-area-inset-bottom)] transition-all shadow-[0_-5px_20px_rgba(0,0,0,0.03)]">
          <div className="flex justify-around items-end h-24 pb-2">
            
            {/* Home Tab */}
            <button
              onClick={() => onTabChange('home')}
              className="flex-1 flex flex-col items-center justify-center gap-1.5 h-full active:scale-95 transition-transform touch-manipulation"
            >
              <Home 
                size={28} 
            ...
            <div className="relative -top-8 flex flex-col items-center justify-center w-24 touch-manipulation">
               <button
                ...
                className={`w-20 h-20 rounded-full flex items-center justify-center shadow-xl shadow-brand-500/30 transition-all active:scale-90 ${
                  activeTab === 'add' 
              ...
              <span className={`text-xs font-medium mt-2 ${activeTab === 'add' ? 'text-brand-600' : 'text-gray-400'}`}>
                记一笔
              </span>
            </div>
