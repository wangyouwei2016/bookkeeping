      ...
      */}
      <div className="w-full h-full flex flex-col relative bg-gray-50
        ...
        <main className="flex-1 w-full relative overflow-y-auto no-scrollbar scroll-smooth">
          ...
          <div className="pb-[calc(14rem+env(safe-area-inset-bottom))] min-h-full">
            {children}
          </div>
        </main>
        ...
        <nav className="absolute bottom-0 left-0 right-0 w-full bg-white/95 backdrop-blur-md border-t border-gray-100 z-50 pb-[env(safe-area-inset-bottom)] transition-all shadow-[0_-5px_20px_rgba(0,0,0,0.03)]">
          <div className="flex justify-around items-end h-40 pb-6">
            ...
            <button
              ...
              className="flex-1 flex flex-col items-center justify-center gap-3 h-full active:scale-95 transition-transform touch-manipulation"
            ...
            >
              ...
              />
              <span className={`text-2xl font-bold ${activeTab === 'home' ? 'text-brand-600' : 'text-gray-400'}`}>
                明细
              </span>
            </button>
            ...
            <div className="relative -top-12 flex flex-col items-center justify-center w-40 touch-manipulation">
               <button
                ...
                className={`w-32 h-32 rounded-full flex items-center justify-center shadow-2xl shadow-brand-300/50 transition-all active:scale-90 ${
                  activeTab === 'add' 
                    ? 'bg-brand-600 ring-8 ring-white' 
                    : 'bg-brand-500 hover:bg-brand-600 ring-8 ring-white'
                }`}
              >
                <PlusCircle size={72} color="white" />
              </button>
              <span className={`text-2xl font-bold mt-4 ${activeTab === 'add' ? 'text-brand-600' : 'text-gray-400'}`}>
                记一笔
              </span>
            </div>
            ...
            >
              <PieChart 
                ...
                strokeWidth={activeTab === 'stats' ? 3 : 2} 
              />
              <span className={`text-2xl font-bold ${activeTab === 'stats' ? 'text-brand-600' : 'text-gray-400'}`}>
                统计
              </span>
            </button>
