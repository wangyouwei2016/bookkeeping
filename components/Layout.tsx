      ...
      */}
      <div className="w-full h-full flex flex-col relative bg-gray-50
        ...
        <main className="flex-1 w-full relative overflow-y-auto no-scrollbar scroll-smooth">
          ...
          <div className="pb-[calc(10rem+env(safe-area-inset-bottom))] min-h-full">
            {children}
          </div>
        </main>
        ...
        <nav className="absolute bottom-0 left-0 right-0 w-full bg-white/95 backdrop-blur-md border-t border-gray-100 z-50 pb-[env(safe-area-inset-bottom)] transition-all shadow-[0_-5px_20px_rgba(0,0,0,0.03)]">
          <div className="flex justify-around items-end h-28 pb-4">
            ...
            {/* Add Tab (Floating) */}
            <div className="relative -top-8 flex flex-col items-center justify-center w-28 touch-manipulation">
               <button
                onClick={() => onTabChange('add')}
                className={`w-24 h-24 rounded-full flex items-center justify-center shadow-xl shadow-brand-300/50 transition-all active:scale-90 ${
                  activeTab === 'add'
