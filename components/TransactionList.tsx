          ...
          <div key={t.id} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 flex items-center justify-between active:bg-gray-50 transition-colors relative group">
            <div className="flex items-center gap-4 overflow-hidden">
              ...
              <div className="min-w-0 space-y-1">
                <div className="font-bold text-gray-800 text-lg truncate">{t.category}</div>
                <div className="text-sm text-gray-400 flex items-center gap-2 flex-wrap">
                  ...
                  <span className={`px-2 py-0.5 rounded-md text-xs font-bold shrink-0 ${t.user === 'husband' ? 'bg-blue-50 text-blue-600' : 'bg-pink-50 text-pink-600'}`}>
                    {formatUser(t.user)}
                  </span>
                </div>
                {t.note && (
                   <div className="text-gray-400 text-sm truncate max-w-[140px] mt-0.5">
                     {t.note}
                   </div>
                )}
              </div>
        ...
        <div ref={loaderRef} className="py-6 flex justify-center items-center text-gray-400 min-h-[60px]">
          {hasMore ? (
            <div className="flex items-center gap-2 text-sm animate-pulse">
             ...
             transactions.length > 0 && <span className="text-xs font-medium opacity-50">—— 到底啦 ——</span>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-gray-900/40 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setItemToDelete(null)}>
          ...
          >
            <div className="p-8 text-center">
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-4">删除确认</h3>
              <p className="text-gray-500 mb-8 leading-relaxed">
                您确定要删除这笔 <span className="font-bold text-gray-800">{itemToDelete.category}</span> 的记录吗？
                <br />
                金额：<span className="font-bold text-gray-800 text-lg">¥{itemToDelete.amount}</span>
              </p>
              
              <div className="flex gap-4">
                <button 
                  onClick={() => setItemToDelete(null)}
                  className="flex-1 py-3 px-4 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold rounded-xl transition-colors text-lg"
                >
                  取消
                </button>
                <button 
                  onClick={confirmDelete}
                  className="flex-1 py-3 px-4 bg-red-500 hover:bg-red-600 text-white font-bold rounded-xl shadow-lg shadow-red-200 transition-colors text-lg"
                >
                  删除
                </button>
              </div>
